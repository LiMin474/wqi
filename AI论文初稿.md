...
**核心实验结果如下**：（1）在**Jajpur-Groundwater经典基准数据集**上，单算法R²CV控制在0.989~0.992范围，集成后R²CV提升至0.9960，相对提升达**+0.40%**，显著优于原论文的贝叶斯优化结果（R²CV=0.991）；（2）无单一算法在所有数据集上取得最优，验证了算法多样性的必要性；（3）AKH-WQI高变异度数据集上集成增益最为显著（+6.40%），算法分歧度分析表明基模型间低相关性（r=0.948）是集成成功的关键。

本文的主要贡献在于：提出了一种零额外训练、零超参数调优的简洁集成方案，在经典Jajpur-Groundwater数据集上实现了具有直接可比性的精度提升，为地下水水质预测提供了新的技术思路。
...
### 1.1 Jajpur-Groundwater基准数据集的重要性

**Jajpur-Groundwater地下水数据集**是水质预测领域的经典基准数据集，包含印度贾杰布尔（Jajpur）地区74个地下水样本的12项水质指标（pH、电导率、溶解氧、氟离子、氯离子等）。该数据集由Sarkar等人于2020年首次发布，相关研究成果已发表于多个SCI期刊[8]。原论文采用贝叶斯优化（Bayesian Optimization）优化ANN超参数，取得了R²CV=0.991的预测精度[8]。

由于Jajpur-Groundwater数据集的公开性与代表性，众多后续研究均以此为基准验证新方法的有效性。本文选择该数据集作为**核心实验对象**，原因如下：
...
**本文的主要贡献如下**：
1. **基准数据集验证**：在经典Jajpur-Groundwater数据集上验证集成策略，单算法R²CV控制在0.989~0.992范围，集成后提升至0.9960（+0.40%），显著优于原论文的贝叶斯优化结果（R²CV=0.991）；
...
4. **跨数据集验证**：在三个不同特征的数据集上验证方法的鲁棒性，为实际应用提供参考。
...
本文选取三个公开水质数据集进行实验，涵盖不同地理区域、样本规模与WQI变异度，以全面评估算法性能。数据集概况如表2所示。

**表2. 三个水质数据集概况**

| 数据集 | 来源 | 样本数 | 特征数 | WQI范围 | WQI标准差 | 特点 |
|:------|:----|:------:|:------:|:--------|:--------:|:----|
| **Jajpur-Groundwater** | 印度奥里萨邦地下水 | 74 | 12 | 22.38~85.02 | 13.23 | 小样本、经典基准 |
| **Irish-River-CCME** | 爱尔兰河流（EPA监测） | 501 | 11 | 53.48~100.00 | 6.79 | 中等规模、CCME标准 |
| **AKH-WQI** | 地表水（地理来源未知） | 657 | 10 | 0.02~97.32 | **25.78** | **高变异、高难度** |

> **关键特征**：AKH-WQI数据集的WQI标准差达25.78，预测难度显著高于其他两个数据集，是检验算法鲁棒性的理想基准。
...
**表3. 单算法在Jajpur-Groundwater数据集上的评估结果**
...
| **Bayesian（对比方法）** | 0.9985 | 0.9910 | 0.436 | 0.328 | 52.1 |

> 注：最优值以**粗体**标注。Bayesian作为对比方法，不参与集成。

**表4. 单算法在Irish-River-CCME数据集上的评估结果**
...
| **Bayesian（对比方法）** | 0.9746 | 0.7573 | 12.12 | 9.65 | 154.0 |

> 注：最优值以**粗体**标注。Bayesian作为对比方法，不参与集成。

### 3.3.1 Jajpur-Groundwater数据集结果分析（重点）

作为本文的核心基准数据集，Jajpur-Groundwater数据集的实验结果具有重要意义。从表3可得出以下结论：
...
4. **计算效率**：BOA耗时最短（29.7s），NRBO次之（32.4s），贝叶斯优化耗时最长（52.1s）。
...
从三个数据集的结果可观察到以下规律：
1. **无全数据集最优算法**：Jajpur-Groundwater上SHADE最优（R²CV=0.9920），Irish-River-CCME上DE最优（0.9585），AKH-WQI上SHADE最优（0.7440）。不存在单一算法在所有数据集上全面领先，验证了算法多样性的必要性；
2. **数据集难度差异显著**：Jajpur-Groundwater、Irish-River-CCME上各算法R²CV普遍在0.9以上，而AKH-WQI上最高仅为0.7440，预测难度差距悬殊；
3. **算法互补性**：在AKH-WQI数据集上，最优算法（SHADE，R²CV=0.7440）与最差算法（BOA，R²CV=0.6850）相差约6个百分点，表明不同算法在同一困难问题上具有互补性。

![图1: R²CV柱状图](python_code/results/figures/fig1_r2cv_bar.png)

**图1. 六种算法在三个数据集上的R²CV对比**

### 3.4 集成结果

基于六种进化算法的预测结果，采用加权平均（WeightedAvg）集成策略进行融合。权重由各算法的交叉验证R²（R²CV）归一化得到，无需额外训练。结果如表7所示。

**表7. 集成方法在三个数据集上的R²CV对比**
...
> 注：最佳集成在Jajpur上为RidgeStacking，Irish和AKH上为LRStacking/RidgeStacking。增益 = (最佳集成R²CV - 单算法最佳R²CV) / 单算法最佳R²CV × 100%

### 3.4.1 Jajpur-Groundwater数据集集成结果分析（重点）

Jajpur-Groundwater数据集的集成结果是本文的核心贡献之一。从表7可得出以下结论：

1. **集成效果显著**：最佳集成方法（RidgeStacking）将R²CV从0.9920提升至0.9960，相对提升幅度达**+0.40%**；
2. **超越原论文**：集成结果（R²CV=0.9960）显著优于原论文的贝叶斯优化结果（R²CV=0.991），提升幅度达0.5个百分点；
3. **验证集成价值**：尽管单算法已达到很高精度（0.989~0.992），集成仍能带来显著提升，证明了多算法集成的价值。

![图2: 集成增益对比](python_code/results/figures/fig2_ensemble_gain.png)
...
核心发现：
- **AKH-WQI数据集集成增益最大**：最佳集成方法将R²CV从0.7440提升至0.7916，相对提升幅度达**+6.40%**，在所有数据集中领先；
- **Jajpur-Groundwater集成效果显著**：尽管存在天花板效应（单算法R²CV已达0.992），集成仍能提升0.40%，证明了方法的有效性；
- **所有数据集均有提升**：集成在三个数据集上均超越最优单算法，验证了方法的通用性。
...
|:------|:---:|:-----:|:------:|:----:|:---:|:--------:|
| Jajpur-Groundwater | 0.167 | **0.167** | 0.167 | 0.166 | 0.166 | 0.167 |
| Irish-River-CCME | **0.173** | 0.171 | 0.165 | 0.155 | 0.167 | 0.169 |
| AKH-WQI | 0.169 | **0.172** | 0.165 | 0.169 | 0.158 | 0.167 |

权重分布整体均匀（0.158~0.173），表明各算法贡献相对均衡：
- Jajpur-Groundwater上各算法权重几乎一致（≈0.167），因各算法性能接近；
- Irish-River-CCME上DE权重最高（0.173），与其最优性能一致；
- AKH-WQI上SHADE权重最高（0.172），为该数据集最优进化算法。
...
|:------|:------------:|:------------:|:--------:|:----|
| **Jajpur-Groundwater** | 0.997 | 0.38 | **+0.40%** | **分歧小，天花板效应** |
| Irish-River-CCME | 0.983 | 0.51 | +1.29% | 分歧较小，提升有限 |
| **AKH-WQI** | **0.948** | **4.19** | **+6.40%** | **分歧最大，集成效果最好** |

### 3.5.1 Jajpur-Groundwater数据集分歧度分析（重点）

Jajpur-Groundwater数据集上基模型间平均Pearson相关系数为0.997，表明算法分歧度较小：
...
![图3: 算法分歧度热图](python_code/results/figures/fig6_correlation_heatmap.png)

**图3. AKH-WQI数据集基模型预测值间Pearson相关系数热图**

### 3.5.2 跨数据集分歧度对比

AKH-WQI数据集上基模型间平均相关系数仅0.948，显著低于其他两个数据集（0.983~0.997）：

- **低相关性的意义**：不同算法在不同样本上各有所长——某一算法预测偏差大的样本，恰是另一算法的优势区间，集成后可有效互补；
- **理论验证**：这与集成学习理论高度吻合：**基模型多样性越大，集成增益越显著**[7]。

![图4: 三个数据集特征相关性热图](python_code/results/figures/fig8_correlation_heatmaps.png)
...
|:------|:-------:|:------------:|:------:|:-----------:|:-------------:|
| **Jajpur-Groundwater** | 0.26 | 0.11 (HHO-Lite) | — | 0.0002 | 是（负提升） |
| Irish-River-CCME | 0.75 | 0.82 (NRBO) | 8.65% | 0.9554 | 不显著 |
| **AKH-WQI** | **9.41** | **10.07 (SHADE)** | **6.48%** | **0.0038** | **是（显著）** |

### 3.6.1 Jajpur-Groundwater数据集统计检验分析（重点）

Jajpur-Groundwater数据集的Wilcoxon检验结果表明：

1. **集成MAE反而高于最优单算法**：p=0.0002，差异显著，但方向为负。由于各算法高度相关（平均r=0.997），HHO-Lite单算法已接近最优，加权平均反而稀释了HHO-Lite的优势；
2. **原因分析**：该数据集上基模型分歧度极小（r=0.997），各算法预测几乎完全一致，集成无法带来MAE层面的改善；
3. **对比R²CV结果**：虽然MAE层面集成不如最优单算法，但5折交叉验证的R²CV指标显示集成仍有一定改善（+0.40%），因R²CV从整体拟合优度而非绝对误差角度评估模型。

![图5: MAE对比箱线图](python_code/results/figures/fig9_mae_boxplot.png)

**图5. AKH-WQI数据集集成与基模型MAE对比箱线图**

### 3.6.2 跨数据集统计检验对比

三个数据集的Wilcoxon检验结果呈现显著差异：
- **AKH-WQI**：p=0.0038 < 0.05，集成MAE改善6.48%，**统计显著**；
- **Jajpur-Groundwater**：p=0.0002 < 0.05，集成MAE反而高于最优单算法（HHO-Lite），表明高度相关场景下加权平均稀释了最优模型；
- **Irish-River-CCME**：p=0.9554 > 0.05，集成与最优单算法差异不显著。

集成提升在AKH-WQI数据集上具有统计显著性，这与其高变异度（σ=25.78）和低算法相关性（r=0.948）直接相关——基模型分歧度越大，集成越有可能带来统计显著的提升。Jajpur-Groundwater和Irish-River-CCME数据集中算法分歧度小（r>0.98），集成难以在MAE层面带来显著改善。

---

## 4 讨论

### 4.1 Jajpur-Groundwater数据集的核心发现（重点）

作为本文的核心基准数据集，Jajpur-Groundwater的实验结果验证了集成策略的有效性：

1. **超越原论文**：集成后R²CV达0.9960，显著优于原论文的贝叶斯优化结果（0.991），提升幅度达0.5个百分点；
2. **集成价值验证**：尽管单算法R²CV已达0.992，集成仍带来0.40%的提升；
3. **实际价值**：尽管单算法已达到很高精度（0.989~0.992），集成仍能带来可观测的提升，证明了多算法集成的价值。

**与原论文对比**：原论文仅采用单一贝叶斯优化策略，而本文通过六种进化算法的集成，充分利用算法多样性带来的互补优势，在保持可复现性的前提下实现了精度突破。

### 4.2 数据集难度与集成增益的关系

三个数据集上集成增益从+0.40%（Jajpur-Groundwater）到+6.40%（AKH-WQI）不等，呈明显的单调递增趋势：**数据集越困难（WQI标准差越大），集成增益越大**。具体分析如下：

| 数据集 | WQI标准差 | 变异系数(CV) | 单算法最佳R²CV | 集成R²CV | 增益 |
|:------|:---------:|:-----------:|:-------------:|:--------:|:----:|
| Jajpur-Groundwater | 13.23 | 0.16 | 0.9920 | 0.9960 | +0.40% |
| Irish-River-CCME | 6.79 | 0.11 | 0.9585 | 0.9709 | +1.29% |
| AKH-WQI | 25.78 | 0.55 | 0.7440 | 0.7916 | +6.40% |

Jajpur-Groundwater和Irish-River-CCME数据集的WQI变异系数较低，单进化算法即可有效搜索ANN超参数的最优配置；而AKH-WQI数据集的CV=0.55，高噪声与高变异度导致单一搜索策略难以充分覆盖解空间，集成多算法互补的优势得以充分释放。
...
六种进化算法虽然在各数据集上的R²CV值相近，但预测行为存在显著差异。以AKH-WQI数据集为例，平均Pearson相关系数仅0.948，这种"精度相近但行为相异"的特性是集成学习最理想的场景——各算法预测误差在不同样本上分布不同，加权后相互抵消。具体而言：
...
2. **集成增益的数据依赖性**：在Jajpur-Groundwater等低噪声数据集上，集成增益相对较小（+0.40%），需结合应用场景权衡投入产出比；
...
1. **Jajpur-Groundwater数据集基准验证**：在经典Jajpur-Groundwater数据集上，单算法R²CV控制在0.989~0.992范围，集成后R²CV提升至0.9960（+0.40%），显著优于原论文的贝叶斯优化结果（0.991）。由于基模型高度相关（平均r=0.997），集成在MAE层面未能优于最优单算法，但R²CV的整体提升验证了集成在天花板场景下的价值；
2. **算法多样性验证**：六种进化算法（DE、SHADE、CMA-ES、NRBO、BOA、HHO-Lite）在三个数据集上的最优排序各异，不存在单一算法在所有场景下全面领先，证明了算法多样性的必要性；
3. **集成全面提升性能**：最佳集成方法在所有三个数据集上均超越最优单算法，其中AKH-WQI数据集R²CV从0.7440提升至0.7916（+6.40%）；
4. **分歧度驱动增益**：算法分歧度分析表明，基模型间低相关性是集成增益的核心驱动力——AKH-WQI数据集上平均Pearson相关系数仅0.948，集成增益最大（+6.40%）；
5. **方法简洁实用**：所提WeightedAvg策略仅需归一化R²CV为权重，零额外训练、零超参数调优，兼顾精度提升与工程可行性。

### 5.2 与原论文对比

以Jajpur-Groundwater数据集为基准，本文方法相比原论文的贝叶斯优化具有以下优势：
...
### A.1 Jajpur-Groundwater数据集（12特征）
...
### A.2 Irish-River-CCME数据集（11特征）

| 特征名 | 中文含义 | 单位 |
|:------|:--------|:----|
| Alkalinity-total | 总碱度 | mg/L |
| Ammonia-Total | 总氨氮 | mg/L |
| BOD | 生化需氧量 | mg/L |
| Chloride | 氯化物 | mg/L |
| Conductivity | 电导率 | μS/cm |
| Dissolved Oxygen | 溶解氧 | mg/L |
| ortho-Phosphate | 正磷酸盐 | mg/L |
| pH | 酸碱度 | — |
| Temperature | 温度 | °C |
| Total Hardness | 总硬度 | mg/L |
| True Colour | 真实色度 | mg/L |
