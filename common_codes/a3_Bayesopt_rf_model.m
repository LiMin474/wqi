function a3_Bayesopt_rf_model (Pred,Resp)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Mtype='Bag';

nfolds=5;
rng(1)
cvs = cvpartition(height(Pred),'KFold',nfolds);

params=Optopt(Pred,Resp,Mtype);
%rng(1)
results = bayesopt(@(paraz)SumSqr(paraz,Pred,Resp,cvs,Mtype,'ita'),params,'ConditionalVariableFcn',@condition,'AcquisitionFunctionName','expected-improvement-plus',...
    'IsObjectiveDeterministic',true,'UseParallel',true,'MaxObjectiveEvaluations',60,'Verbose',1,'PlotFcn',[]);
A1 = results.XAtMinObjective;

%%%% Best model %%%
[~,output]=SumSqr(A1,Pred,Resp,cvs,Mtype,'final');

Mdl=output{3};

A1.Method=Mdl.Method;A1=movevars(A1,'Method','Before',1);
A1.Resp=categorical("RFWQI");A1.R2=output{1};A1.R2CV=output{2};
A1=movevars(A1,'Resp','Before',1);

name='random_forest.mat';
Opttable_rf=A1;
Modells_rf=Mdl;
save (name, 'Opttable_rf', 'Modells_rf')
disp('');
...
function hyperset=Optopt(XX,Res,Modeltype)
hyperset = hyperparameters('fitrensemble',XX,Res,'Tree');
