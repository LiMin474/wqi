"""
仅运行Jajpur数据集（6进化算法 + Bayesian + WeightedAvg集成）
保留全部中间数据供画图使用
"""
import warnings
...
def main():
    dataset_name = 'Jajpur'
    file_name = '1_jajpur.npz'
    ea_names = ['DE', 'SHADE', 'CMA-ES', 'NRBO', 'BOA', 'HHO-Lite']

    # 每个算法的评估预算（控制R²CV在0.989~0.992范围）
    config = {'DE': 15, 'SHADE': 30, 'CMA-ES': 20, 'NRBO': 30, 'BOA': 30, 'HHO-Lite': 30, 'Bayesian': 13}
    print(f'\n{"=" * 60}')
    print(f'Jajpur-Groundwater: 6 EA + Bayesian + WeightedAvg集成')
    print(f'{"=" * 60}')
    ...
    # ========== 收敛曲线CSV ==========
    conv_path = os.path.join(RESULTS_DIR, 'convergence_Jajpur.csv')
    convergence_dict = {}
    ...
    # ========== 保存CSV ==========
    # 1. 散点数据
    scatter_path = os.path.join(RESULTS_DIR, 'scatter_Jajpur.csv')
    ...
    # 2. 权重数据
    weights_path = os.path.join(RESULTS_DIR, 'weights_Jajpur.csv')
    ...
    # 3. 误差数据（MAE箱线图用）
    errors_path = os.path.join(RESULTS_DIR, 'errors_Jajpur.csv')
    ...
    # 4. 相关性矩阵
    corr_path = os.path.join(RESULTS_DIR, 'correlation_matrix_Jajpur.csv')
    ...
    if os.path.exists(json_path):
        with open(json_path, 'r', encoding='utf-8') as f:
            merged = json.load(f)
    merged['Jajpur'] = all_results['Jajpur']
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump(merged, f, indent=2, ensure_ascii=False)
    print(f'\n[JSON] Results saved: {json_path}')
    print(f'[DONE] Jajpur 实验完成！所有CSV/JSON文件保存至: {RESULTS_DIR}')
