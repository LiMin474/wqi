"""
Generate 7 figures for Irish-River-CCME dataset
Target: 6 evolutionary algorithms + WeightedAvg ensemble
Output: 2_results/figures/
"""
import pandas as pd
import numpy as np
...
def plot_fig1():
    ...
    ax.set_title('Single Algorithm R²CV vs WeightedAvg Ensemble (Irish-River-CCME)',
                 fontsize=12)
...
def plot_fig_conv():
    ...
    conv = pd.read_csv(os.path.join(RESULTS_DIR, 'convergence_Irish.csv'))
    ...
    ax_main.set_title('Convergence Curves on Irish Dataset (6 Evolutionary Algorithms)',
                      fontsize=12)
...
def plot_fig_pareto():
    ...
    ax.set_xlim(0, None)
    ax.set_ylim(0.93, 1.005)
    ax.set_title('Accuracy vs Computational Efficiency (Irish-River-CCME)', fontsize=12)
    ax.legend(loc='lower right', fontsize=10)
...
def plot_fig_scatter():
    ...
    df = pd.read_csv(os.path.join(RESULTS_DIR, 'scatter_Irish.csv'))
    ...
    ax.set_title('Ensemble Prediction vs Actual WQI (Irish, 501 Samples)', fontsize=12)
...
def plot_fig_algo_corr():
    ...
    df = pd.read_csv(os.path.join(RESULTS_DIR, 'correlation_matrix_Irish.csv'), index_col=0)
    ...
    ax.set_title('Algorithm Prediction Correlation (Irish Dataset)', fontsize=12)
...
def plot_fig_weights():
    ...
    ax.set_title('WeightedAvg Ensemble Weights (Irish Dataset)', fontsize=12)
...
def plot_fig_feat_corr():
    ...
    data = np.load(os.path.join(RESULTS_DIR, '..', 'datasets', '2_irish_river.npz'), allow_pickle=True)
    X = data['X']

    FEAT_NAMES = ['pH', 'EC', 'DO', 'Cl', 'NO3', 'SO4',
                  'PO4', 'Temp', 'Cond', 'Turb', 'BOD']
    ...
    ax.set_title('Feature Correlation Matrix (Irish-River-CCME, 11 Indicators + WQI)',
                 fontsize=12, pad=15)
    fig.tight_layout()
...
if __name__ == '__main__':
    print('=' * 50)
    print('Generating 7 figures for Irish dataset...')
    print('=' * 50)
