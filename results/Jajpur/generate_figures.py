"""
Generate 8 figures for Jajpur-Groundwater dataset
Target: 6 evolutionary algorithms + WeightedAvg ensemble
Output: 1_results/figures/
"""
import pandas as pd
import numpy as np
...
# ════════════════════════════════════════════════
# Figure 1: R²CV bar chart with best single and ensemble
# ════════════════════════════════════════════════
def plot_fig1():
    global fig
    ...
    ax.set_title('Single Algorithm R²CV vs WeightedAvg Ensemble (Jajpur-Groundwater)',
                 fontsize=12)
...
# ════════════════════════════════════════════════
# Figure 3: Convergence curves (zoomed to highlight top)
# ════════════════════════════════════════════════
def plot_fig_conv():
    global fig
    fig, ax_main = plt.subplots(figsize=(10, 6))

    conv = pd.read_csv(os.path.join(RESULTS_DIR, 'convergence_Jajpur.csv'))

    # Pad all sequences to 20 evaluations
    MAX_X = 15
    conv_order = ['DE', 'SHADE', 'CMA-ES', 'NRBO', 'BOA', 'HHO-Lite']

    def pad_to_20(series):
    ...
    x_20 = np.arange(1, MAX_X + 1)

    # Full view (main)
    for algo in conv_order:
        vals = pad_to_20(conv[algo])
        ax_main.plot(x_20, vals, label=algo,
    ...
    ax_main.set_title('Convergence Curves on Jajpur Dataset (6 Evolutionary Algorithms)',
                      fontsize=12)
...
# ════════════════════════════════════════════════
# Figure 3: Pareto chart (R²CV vs Time)
# ════════════════════════════════════════════════
def plot_fig_pareto():
    global fig
    ...
    # Ensemble (no training time, marked at ~0.5s for visibility)
    wa = DATA['ensemble_results']['WeightedAvg']['R2CV']
    ...
    ax.set_xlim(0, None)
    ax.set_ylim(0.97, 1.005)
    ax.set_title('Accuracy vs Computational Efficiency (Jajpur)', fontsize=12)
    ax.legend(loc='lower right', fontsize=10)
...
def plot_fig_scatter():
    ...
    ax.scatter(df['Actual'], df['WeightedAvg_Pred'], s=35, c='#3498DB',
               alpha=0.65, edgecolors='black', linewidth=0.3, zorder=5)

    ax.set_xlim(lo, hi)
    ax.set_ylim(lo, hi)
    ax.set_xlabel('Actual WQI', fontsize=11)
    ax.set_ylabel('WeightedAvg Predicted WQI', fontsize=11)
    ax.set_title('Ensemble Prediction vs Actual WQI (Jajpur, 74 Samples)', fontsize=12)
...
def plot_fig_algo_corr():
    ...
    ax.set_title('Algorithm Prediction Correlation (Jajpur Dataset)', fontsize=12)
...
def plot_fig_weights():
    ...
    ax.set_title('WeightedAvg Ensemble Weights (Jajpur Dataset)', fontsize=12)
...
# ════════════════════════════════════════════════
# Figure 7: Feature correlation heatmap (12 indicators + WQI)
# ════════════════════════════════════════════════
def plot_fig_feat_corr():
    global fig
    fig, ax = plt.subplots(figsize=(11, 9))

    data = np.load(os.path.join(RESULTS_DIR, '..', 'datasets', '1_jajpur.npz'), allow_pickle=True)
    X = data['X']

    FEAT_NAMES = ['pH', 'EC', 'DO', 'F', 'Cl', 'NO3',
                  'SO4', 'PO4', 'U', 'CaH', 'MgH', 'HCO3']
    ...
    ax.set_title('Feature Correlation Matrix (Jajpur-Groundwater, 12 Indicators + WQI)',
                 fontsize=12, pad=15)
    fig.tight_layout()
...
if __name__ == '__main__':
    print('=' * 50)
    print('Generating 7 figures for Jajpur dataset...')
    print('=' * 50)
